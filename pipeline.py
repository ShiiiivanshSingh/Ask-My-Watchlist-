...
class SemanticCache:
    ...
    def lookup(self, query):
        ...
        if hits and hits[0][1] >= self.threshold:
            hit = hits[0][0]
            return hit["answer"], hits[0][1], hit.get("context"), hit.get("source")
        return None, (hits[0][1] if hits else 0.0), None, None

    def store(self, query, answer, source, context=None):
        vec = self.embedder.encode(query)[0]
        record = {
            "id": str(uuid.uuid4()),
            "query": query,
    ...
    def clear(self):
        count = len(self.index.records)
        self.index = VectorIndex(self.embedder.get_sentence_embedding_dimension())
        self.index.save(self.path)
        return count
...
class Pipeline:
    ...
    def ask(self, query):
        cached_answer, score, cached_context, cached_from = self.cache.lookup(query)
        ...
        if context is not None:
            ...
            self.cache.store(query, answer, "kb", context=context)
            return {"answer": answer, "source": "kb", "score": kb_score, "context": context}

        no_match_note = (
            "No entry for this exists in the user's personal watch/review log. "
            "Say plainly that there's no record of it, rather than answering from "
            "general knowledge as if it were a personal fact."
        )
        answer = self.api_client.answer_with_context(query, no_match_note)
        self.cache.store(query, answer, "api", context=no_match_note)
        return {"answer": answer, "source": "api", "score": 0.0, "context": no_match_note}

    def clear_cache(self):
        return self.cache.clear()

    def cache_size(self):
        return len(self.cache.index.records)